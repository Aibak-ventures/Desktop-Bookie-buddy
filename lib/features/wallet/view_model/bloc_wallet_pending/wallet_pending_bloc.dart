...
import 'package:bookie_buddy_web/features/wallet/models/ledger_pendings_grouped_model/ledger_pendings_grouped_model.dart';
...
class WalletPendingBloc extends Bloc<WalletPendingEvent, WalletPendingState> {
  ...
  WalletPendingBloc() : super(const _Loading()) {
    on<_LoadPending>(_onLoadPending);
    on<_LoadNextPagePending>(_onLoadNextPagePending);
  }

  Future<void> _onLoadPending(
      _LoadPending event, Emitter<WalletPendingState> emit) async {
    if (state is! _Loading) emit(const _Loading());
    try {
      final result = await _repository.getPendingPagination(
        page: 1,
      ...
      emit(
        _Loaded(
          walletPending: _groupByDate(result.data),
  ...
  Future<void> _onLoadNextPagePending(
      _LoadNextPagePending event, Emitter<WalletPendingState> emit) async {
    ...
    try {
      ...
      final grouped = _groupByDate(result.data, s.walletPending);
