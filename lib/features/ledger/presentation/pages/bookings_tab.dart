...
import 'package:bookie_buddy_web/features/ledger/presentation/widgets/ledger_bookings_grouped_container.dart';
...
class _BookingsTabState extends State<BookingsTab> {
  ...
  void _onScroll() {
    // Handle pagination
    final bloc = context.read<LedgerBookingsBloc>();
    if (bloc.state.isLoaded) {
      ...
      if (!state.isPaginating &&
        ...
        bloc.add(const LedgerBookingsEvent.loadNextPageBookings());
      }
    }
            ...
            loaded: (
              ...
              return RepaintBoundary(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    if (index == ledgerBookings.length) {
                      return const LedgerBookingListTileShimmer();
                    }
                    final groupedData = ledgerBookings[index];
                    final key = _groupKeysWithTotal
                        .putIfAbsent(
