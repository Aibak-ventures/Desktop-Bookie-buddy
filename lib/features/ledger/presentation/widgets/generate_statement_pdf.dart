...
import 'package:bookie_buddy_web/features/ledger/domain/models/ledger_summary_model.dart';
...
class LedgerPDFService {
  ...
  static Future<void> shareLedgerInvoice({
    ...
    if (isExcel) {
      try {
        final filePath = await getIt.get<DownloadLedgerInvoiceUseCase>()(
          fromDate: fromDate,
          toDate: toDate,
          type: typeValue,
          clientId: clientId,
    ...
    try {
      final entries = invoiceData ??
          await getIt.get<GetLedgerInvoiceDataUseCase>()(
            fromDate: fromDate,
            toDate: toDate,
            type: typeValue,
            clientId: clientId,
