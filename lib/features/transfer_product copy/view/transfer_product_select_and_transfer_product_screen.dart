                        ...
                        onNotification: (scrollInfo) {
                          if (nextPageUrl != null &&
                            ...
                            context.read<MatchingProductBloc>().add(
                              const MatchingProductEvent.loadNextPageMatchingProducts(),
                            );
                          }
                          return false;
                        },
            ...
            child: Padding(
              ...
              child: BlocBuilder<MatchingProductBloc, MatchingProductState>(
                builder: (context, state) {
                  final isTransferButtonVisible = state.maybeMap(
                    orElse: () => false,
                    loaded: (value) => value.products.isNotEmpty,
                  );
                  final isCreateButtonVisible = state.maybeMap(
                    orElse: () => false,
                    loaded: (value) => value.products.any(
                      (product) =>
                          product.name == fromProduct.name &&
                          product.category == fromProduct.category &&
                          product.model == fromProduct.model &&
                          product.color == fromProduct.color,
                    ),
                  );
                  log(
                    'isCreateButtonVisible: $isCreateButtonVisible, isTransferButtonVisible: $isTransferButtonVisible',
                  );
                  return Column(
                    spacing: 10,
                    children: [
                      ...
                      Visibility(
                        visible: isTransferButtonVisible,
                        child: SizedBox(
                          height: 40.w,
                          width: double.infinity,
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                final transferQuantity = quantityController.text.toIntOrNull();
